...
3 things Gus wants that are technically impossible 
•	Ore "smelling". Robots, especially Go2's, can't find ore just by smelling for it. They need to take physical soil samples, chemically analyze the soils with chemicals, acids and other such stuff.
•	60km Robots: Robots moving at 60km, especially legged ones, is not possible. Firstly, its just too fast. Also the speed to navigate at that speed, with little to no latency, UNDERGROUND? Any loss in signal will result in expensive consequences.
•	AI communications protocol: I'm not too sure if this can be achieved with current tech. But AIs talking to each other in a way people can't understand or intercept sounds terrifying.

3 things that are genuinely feasible 
•	Autonomous blasting: Robots setting mining charges and blasting is a technology that is used in real mine sites.
•	Dog army: Having a group of robots scan areas and communicate with each other is a technology that is feasible.
•	Predictive ore drilling AI: Training AI to predict drilling locations with 40 years of survey data can be created today.
